...
export default async function TagPage({ params, searchParams }: PageProps<'/tag/[slug]'>) {
  ...
  return <><div className="page-head"><span className="kicker">Argomento</span><h1>{t.name}</h1><p>{desc} <span className="count">{total} articoli</span></p></div><div className="layout-sidebar"><div><ArticleList articles={articles} total={total} page={page} perPage={perPage} basePath={`/tag/${t.slug}`} /></div><Sidebar /></div></>;
}
