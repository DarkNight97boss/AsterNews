...
import { getPublishedReports, getZones } from '@/lib/queries';
...
export default async function ReportsPage() {
  const [reports, zones] = await Promise.all([getPublishedReports(), getZones()]);
  return (
    <>
      ...
      <div className="layout-sidebar">
        <div className="reports">
          ...
          {reports.map((r) => { const z = zones.find((x) => x.id === r.zoneId); return (
        ...
        <aside className="sidebar"><div className="widget"><h3 className="widget-title">Invia una segnalazione</h3><ReportForm zones={zones.map((z) => ({ id: z.id, name: z.name }))} /></div></aside>
      </div>
    </>
  );
}
