...
export async function GET(_req: Request, { params }: { params: Promise<{ categorySlug: string; articleSlug: string }> }) {
  ...
  const body = (await enhanceContent(stripCircles(a.content))).replace(/<iframe[\s\S]*?<\/iframe>/g, '').replace(/<script[\s\S]*?<\/script>/g, '').replace(/<img([^>]*)>/g, '<img loading="lazy" decoding="async"$1>');
