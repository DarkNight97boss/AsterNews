...
export default async function ParticipationPage() {
  ...
  return (
    <>
      ...
      {groups.filter((g) => g.pending.length > 0).map(({ k, pending }) => <div className="panel" key={k.kind}><div className="panel-title">{k.label} ({pending.length})</div>{pending.map((r) => <div key={r.id} className="reply-item"><p><b>{r.data[k.who] || 'Anonimo'}</b>{r.data.email ? <> · <a href={`mailto:${r.data.email}`}>{r.data.email}</a></> : null}{r.data.title ? <> · «{r.data.title}»</> : null}{r.ref && r.data.articleTitle ? <> · su <Link href={`/admin/articoli/${r.ref}`}>{r.data.articleTitle}</Link></> : null}</p>{String(r.data[k.body] ?? '').startsWith('data:image/') ? <img src={r.data[k.body]} alt="Messaggio scritto a mano" style={{ maxWidth: 420, border: '1px solid #ddd', borderRadius: 6 }} /> : <blockquote>{String(r.data[k.body] ?? '').slice(0, 1600)}</blockquote>}<div style={{ display: 'flex', gap: 6 }}><ActionButton action={moderateRecordAction.bind(null, r.id, 'approved')}>Pubblica</ActionButton><ActionButton action={moderateRecordAction.bind(null, r.id, 'rejected')} className="btn btn-ghost btn-sm">Rifiuta</ActionButton><ActionButton action={deleteRecordAction.bind(null, r.id)} className="btn btn-ghost btn-sm">Elimina</ActionButton></div></div>)}</div>)}
