...
export default async function AdminLayout({ children }: LayoutProps<'/admin'>) {
  ...
  return <AdminShell adapt={adapt} solo={solo} used={usage.used} mature={mature} user={user} permissions={permissionsOf(user)} unread={unread} quiet={{ from: settings.rhythm?.quietFrom, to: settings.rhythm?.quietTo }} reviewCount={can(user, 'article.publish') ? counts.review : 0} pendingComments={st.pendingComments} pendingEvents={st.pendingEvents} newReports={st.newReports}>{children}</AdminShell>;
}
