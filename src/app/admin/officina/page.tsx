...
export default async function WorkshopPage({ searchParams }: PageProps<'/admin/officina'>) {
  ...
  const [seedsR, sessions, data, graveyardAll, expiringAll, withData] = await Promise.all([listRecords<{ text: string }>('seed', { owner: me.id }), listRecords<{ words: number; minutes: number; day: string }>('session', { owner: me.id, limit: 400 }), listRecords<LiveDatum>('datum', { order: 'old' }), listArticles({ extraHas: 'abandoned', includeCircles: true, ...(any ? {} : { authorId: me.id }) }, 'updated', 200), listArticles({ status: 'published', extraHas: 'expiring', includeCircles: true, ...(any ? {} : { authorId: me.id }) }, 'updated', 300), listArticles({ q: '{{dato:', includeCircles: true }, 'updated', 300)]);
  const seeds = seedsR.map((s) => ({ id: s.id, text: s.data.text, createdAt: s.createdAt })); const themes = ripeThemes(seeds).map((t) => ({ theme: t.theme, ids: t.notes.map((n) => n.id) }));
