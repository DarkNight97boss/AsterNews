...
export async function ArticleFanpage({ a, cat, author, tags, rel, comments, zone, moderated, googleNewsUrl, siteName }: Props) {
  const [cats, videos, mostAll] = await Promise.all([getCategories(), listPublished({ format: 'video', excludeIds: [a.id] }, 1), getMostRead(6)]);
  const liveUpdates = [...a.liveUpdates].sort((x, y) => y.time.localeCompare(x.time));
  const video = videos[0]; const most = mostAll.filter((m) => m.id !== a.id).slice(0, 5);
  return (
    <div className="fp-article">
      ...
      <aside className="fp-article-side">
        {rel.slice(1, 3).map((r) => <FpPhotoCard key={r.id} a={r} cats={cats} />)}
        {video && <FpPhotoCard a={video} cats={cats} badge="▶ Video" />}
