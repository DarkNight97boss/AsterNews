...
/** Layout "griglia" (stile Fanpage): apertura con foto grande, ultime notizie con orari. */
export function HomeGrid({ d }: { d: HomeData }) {
    ...
    <>
      {lead && (
        <section className="hero-grid">
          ...
          <div className="hero-grid-side">{side.slice(0, 2).map((a) => <ArticleCard key={a.id} article={a} variant="md" />)}</div>
        </section>
      )}
          ...
          {d.latest.map((a) => (
            <article key={a.id} className="card card-sm">
              ...
              <div className="card-body">
                <div className="meta"><span className="time-badge">{shortTime(a)}</span> <b>{category(a.categoryId)?.name}</b></div>
                <h3 className="card-title"><Link href={articleUrl(a)}>{a.title}</Link></h3>
              </div>
            </article>
          ))}
        </div>
      ...
      {d.dossierCat && d.dossierLead && (
        <section className="section"><div className="section-title"><h2>{d.dossierCat.name}</h2><Link href={`/${d.dossierCat.slug}`}>Vai alla sezione →</Link></div>
          <div className="grid grid-4">{[d.dossierLead, ...d.dossierRest].map((a) => <ArticleCard key={a.id} article={a} variant="sm" />)}</div></section>
      )}
      <CommonSections d={d} />
    </>
  );
