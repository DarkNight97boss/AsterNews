...
export function Paywall({ articleId, premiumOnly, price, free, children }: { articleId: string; premiumOnly: boolean; price: number; free: number; children: ReactNode }) {
  const [state, setState] = useState<{ allowed: boolean; left: number; premiumRequired: boolean } | null>(premiumOnly ? { allowed: false, left: 0, premiumRequired: true } : null);
  ...
  return (
    <div className="paywall">
      ...
      <div className="paywall-box">
        <h3>{state.premiumRequired ? 'Articolo riservato agli abbonati' : `Hai letto i ${free} articoli gratuiti del mese`}</h3>
        <p>Sostieni il giornalismo locale: con {price.toFixed(2).replace('.', ',')} € al mese leggi tutto, senza limiti, e disdici quando vuoi.</p>
