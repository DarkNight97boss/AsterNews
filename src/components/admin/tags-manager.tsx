...
export function TagsManager({ tags, counts }: { tags: Tag[]; counts: Record<string, number> }) {
  const router = useRouter();
  const [q, setQ] = useState(''); const [name, setName] = useState(''); const [editing, setEditing] = useState<Tag | null>(null);
  ...
  return (
    <>
      ...
      <div className="filters"><input className="input grow" placeholder="Cerca tag..." value={q} onChange={(e) => setQ(e.target.value)} /></div>
