...
export function ArticleEditor({ initial, isNew, isPublic, categories, zones, tags: allTags, users, media, permissions, seoCtx, siteUrl, maxLinks, meId }: Props) {
  ...
  return (
    <>
      ...
      <div className="editor-tools"><button type="button" className={`btn btn-sm ${focus ? 'btn-dark' : 'btn-ghost'}`} onClick={() => setFocus(!focus)} title="Schermo pulito, solo testo">{focus ? '✕ Esci da Focus' : '◎ Modalità Focus'}</button><span className="help">{words} parole{extra.wordsTarget ? ` / obiettivo ${extra.wordsTarget}` : ''} · {Math.max(1, Math.round(words / 200))} min di lettura</span><DictateButton onText={(t) => set('content', a.content + `\n<p>${t.replace(/</g, '&lt;')}</p>`)} />{focus && <input className="input" style={{ width: 120 }} type="number" placeholder="Obiettivo parole" value={extra.wordsTarget ?? ''} onChange={(e) => setExtra({ wordsTarget: Number(e.target.value) || undefined })} />}</div>
