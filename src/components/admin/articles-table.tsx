...
interface Filters { q: string; status: string; categoria: string; autore: string; ordina: string; dir: string }
interface Props { articles: Article[]; total: number; page: number; perPage: number; counts: Record<string, number>; categories: Category[]; users: User[]; me: User; permissions: Permission[]; filters: Filters }
...
export function ArticlesTable({ articles, total, page, perPage, counts, categories, users, me, permissions, filters }: Props) {
  ...
  return (
    <>
      <form className="filters" onSubmit={(e) => { e.preventDefault(); go({ q }); }}>
        <input className="input grow" placeholder="Cerca per titolo o slug…" value={q} onChange={(e) => setQ(e.target.value)} />
      ...
      <div className="table-wrap"><table className="table">
        ...
        <tbody>
          ...
          {articles.length === 0 && <tr><td colSpan={10}><div className="empty"><h3>Nessun articolo</h3><p>Prova a cambiare i filtri o crea un nuovo articolo.</p></div></td></tr>}
        </tbody></table></div>
