...
export async function aiFactCheckAction(content: string): Promise<R<{ claim: string; note: string; severity: 'info' | 'warn'; sources?: string[] }[]>> {
  const me = await getCurrentUser(); if (me) { try { const { askWithSearch } = await import('./ai'); const raw = await askWithSearch(`Verifica i fatti di questo testo giornalistico italiano cercando online le fonti. Individua fino a 6 affermazioni verificabili (cifre, date, nomi, cariche, citazioni) e per ciascuna indica: "claim" (frase citata), "note" (cosa risulta dalle fonti: confermato, smentito, non trovato, con dettaglio), "severity" ("warn" se smentito o non trovato, altrimenti "info"), "sources" (array di URL delle fonti consultate). Rispondi SOLO con un array JSON.\n\nTesto:\n${clip(stripHtml(content), 8000)}`, { maxTokens: 3000, maxUses: 6 }); const m = raw.match(/\[[\s\S]*\]/); if (m) return { ok: true, data: JSON.parse(m[0]) }; } catch { /* ripiego senza ricerca */ } }
