...
export async function synthesize(text: string, voiceOverride?: string): Promise<Buffer> {
  ...
  if (provider === 'elevenlabs') {
    const voice = voiceOverride || s.ttsVoice || 'JBFqnCBsd6RMkjVDRZzb';
  ...
  const parts: Buffer[] = [];
  for (const c of chunks.slice(0, 6)) { const r = await fetch('https://api.openai.com/v1/audio/speech', { method: 'POST', headers: { Authorization: `Bearer ${key}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ model: 'gpt-4o-mini-tts', voice: voiceOverride || s.ttsVoice || 'alloy', input: c, response_format: 'mp3' }), signal: AbortSignal.timeout(120_000) }); if (!r.ok) throw new Error(`OpenAI TTS ${r.status}: ${(await r.text()).slice(0, 120)}`); parts.push(Buffer.from(await r.arrayBuffer())); }
