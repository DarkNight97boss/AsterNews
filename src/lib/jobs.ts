...
export async function runMinuteJobs(): Promise<JobReport> {
  ...
  try { const hourRome = Number(new Date().toLocaleString('it-IT', { hour: 'numeric', hour12: false, timeZone: 'Europe/Rome' })); if (s.briefing?.enabled && hourRome >= (s.briefing.hour ?? 6) && hourRome < (s.briefing.hour ?? 6) + 3) { const { runBriefing } = await import('./briefing'); const r = await runBriefing(); if (!/già fatta|disattivata/.test(r)) steps.rassegnaMattutina = r; } } catch (e) { steps.rassegnaMattutina = 'errore: ' + (e as Error).message; }
