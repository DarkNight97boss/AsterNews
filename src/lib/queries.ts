...
export const search = async (q: string, limit = 50, offset = 0, opts: { categoryId?: string; from?: string; to?: string } = {}) => { const r0 = await repo.searchArticles(q, limit, offset, { ...opts, synonyms: await synonyms() }); const r = { ...r0, items: r0.items.filter((a) => !a.extra?.circle) }; if (r.items.length >= 5 || offset > 0 || opts.categoryId) return r; try { const { semanticSearch } = await import('./embeddings'); const extra = await semanticSearch(q, limit - r.items.length, r.items.map((a) => a.id)); return extra.length ? { ...r, items: [...r.items, ...extra], total: r.total + extra.length } : r; } catch { return r; } };
