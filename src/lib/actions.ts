...
export async function setCookieConsentAction(value: 'all' | 'necessary'): Promise<void> { if (value !== 'all' && value !== 'necessary') return; const s = await getSettings(); const version = s.privacy?.policyVersion ?? 1; (await cookies()).set('cookie_consent', `${value}:v${version}`, { path: '/', maxAge: 60 * 60 * 24 * 180, sameSite: 'lax' }); try { const { headers } = await import('next/headers'); const h = await headers(); const ip = (h.get('x-forwarded-for') ?? '').split(',')[0].trim(); const { createHash } = await import('node:crypto'); const x3 = await import('./repo-extra3'); if ((await import('./ratelimit')).checkLimit(await (await import('./ratelimit')).ipKey('consent'), 5, 600_000).ok) await x3.insertConsent(value, version, ip ? createHash('sha256').update(ip + (await (await import('./auth')).getSecret())).digest('hex').slice(0, 24) : '', h.get('user-agent') ?? ''); } catch { /* il consenso resta nel cookie */ } }
