...
export function rowToArticle(r: Row): Article {
  return {
    ...
    assignedTo: String(r.assigned_to ?? ''), deadline: (r.deadline as string | null) ?? null, premium: !!r.premium, editionId: String(r.edition_id ?? ''), faq: pj<{ q: string; a: string }[]>(r.faq, []), socialText: String(r.social_text ?? ''), deletedAt: (r.deleted_at as string | null) ?? null, coauthorIds: pj<string[]>(r.coauthor_ids, []), byline: String(r.byline ?? ''),
  };
}
...
const UPSERT_ARTICLE = `INSERT INTO articles (id, slug, kicker, title, subtitle, excerpt, content, cover_image, cover_caption, category_id, author_id, zone_id, address, status, format, video_url, gallery, live_updates, live_active, featured, breaking, sponsored, allow_comments, seo, tag_ids, views, published_at, scheduled_at, created_at, updated_at, seo_score, seo_report, legacy_url, wp_id, search_text, assigned_to, deadline, premium, edition_id, faq, social_text, coauthor_ids, byline)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
...
export async function insertComment(c: Comment): Promise<void> { await run('INSERT INTO comments (id, article_id, author_name, email, body, status, created_at, reader_id, parent_id, flags, votes, staff) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)', [c.id, c.articleId, c.authorName, c.email, c.body, c.status, c.createdAt, c.readerId ?? '', c.parentId ?? '', c.flags ?? 0, c.votes ?? 0, c.staff ? 1 : 0]); }
