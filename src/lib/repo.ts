...
export function rowToArticle(r: Row): Article {
  return {
    ...
    assignedTo: String(r.assigned_to ?? ''), deadline: (r.deadline as string | null) ?? null, premium: !!r.premium, editionId: String(r.edition_id ?? ''), faq: pj<{ q: string; a: string }[]>(r.faq, []),
  };
}
...
export async function bulkUpsertArticles(list: { a: Article; wpId?: string }[]): Promise<void> {
  ...
  const COLS = 40;
  const cols = 'id, slug, kicker, title, subtitle, excerpt, content, cover_image, cover_caption, category_id, author_id, zone_id, address, status, format, video_url, gallery, live_updates, live_active, featured, breaking, sponsored, allow_comments, seo, tag_ids, views, published_at, scheduled_at, created_at, updated_at, seo_score, seo_report, legacy_url, wp_id, search_text, assigned_to, deadline, premium, edition_id, faq';
  const stmts: Stmt[] = [];
