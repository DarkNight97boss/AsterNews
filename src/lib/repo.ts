...
export function rowToArticle(r: Row): Article {
  return {
    ...
    assignedTo: String(r.assigned_to ?? ''), deadline: (r.deadline as string | null) ?? null, premium: !!r.premium, editionId: String(r.edition_id ?? ''), faq: pj<{ q: string; a: string }[]>(r.faq, []), socialText: String(r.social_text ?? ''),
  };
}
...
export const rowToComment = (r: Row): Comment => ({ id: String(r.id), articleId: String(r.article_id), authorName: String(r.author_name ?? ''), email: String(r.email ?? ''), body: String(r.body ?? ''), status: r.status as Comment['status'], createdAt: String(r.created_at ?? ''), readerId: String(r.reader_id ?? ''), parentId: String(r.parent_id ?? ''), flags: Number(r.flags ?? 0) });
export const rowToMedia = (r: Row): MediaItem => ({ id: String(r.id), name: String(r.name ?? ''), url: String(r.url), alt: String(r.alt ?? ''), type: (r.type as MediaItem['type']) ?? 'image', size: Number(r.size ?? 0), uploadedBy: String(r.uploaded_by ?? ''), createdAt: String(r.created_at ?? ''), provider: String(r.provider ?? ''), path: String(r.path ?? ''), width: Number(r.width ?? 0), height: Number(r.height ?? 0), variants: pj<Record<string, string>>(r.variants, {}), focalX: Number(r.focal_x ?? 0.5), focalY: Number(r.focal_y ?? 0.5) });
...
export async function insertComment(c: Comment): Promise<void> { await run('INSERT INTO comments (id, article_id, author_name, email, body, status, created_at, reader_id, parent_id, flags) VALUES (?,?,?,?,?,?,?,?,?,?)', [c.id, c.articleId, c.authorName, c.email, c.body, c.status, c.createdAt, c.readerId ?? '', c.parentId ?? '', c.flags ?? 0]); }
...
export async function insertMedia(m: MediaItem): Promise<void> { await run('INSERT INTO media (id, name, url, alt, type, size, uploaded_by, created_at, provider, path, width, height, variants, focal_x, focal_y) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [m.id, m.name, m.url, m.alt, m.type, m.size, m.uploadedBy, m.createdAt, m.provider ?? '', m.path ?? '', m.width ?? 0, m.height ?? 0, j(m.variants ?? {}), m.focalX ?? 0.5, m.focalY ?? 0.5]); }
export async function updateMediaRow(m: MediaItem): Promise<void> { await run('UPDATE media SET name = ?, alt = ?, focal_x = ?, focal_y = ? WHERE id = ?', [m.name, m.alt, m.focalX ?? 0.5, m.focalY ?? 0.5, m.id]); }
...
export async function bulkUpsertArticles(list: { a: Article; wpId?: string }[]): Promise<void> {
  ...
  const COLS = 41;
  const cols = 'id, slug, kicker, title, subtitle, excerpt, content, cover_image, cover_caption, category_id, author_id, zone_id, address, status, format, video_url, gallery, live_updates, live_active, featured, breaking, sponsored, allow_comments, seo, tag_ids, views, published_at, scheduled_at, created_at, updated_at, seo_score, seo_report, legacy_url, wp_id, search_text, assigned_to, deadline, premium, edition_id, faq, social_text';
  const stmts: Stmt[] = [];
