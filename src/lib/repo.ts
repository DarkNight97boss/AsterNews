...
// ---------------- Job di importazione ----------------
export interface ImportJob { id: string; source: 'wxr' | 'rest' | 'feed' | 'drupal' | 'joomla'; status: 'queued' | 'running' | 'done' | 'failed' | 'cancelled'; options: Record<string, unknown>; file: string; total: number; processed: number; imported: number; skipped: number; errors: string[]; message: string; cursor: string; createdAt: string; updatedAt: string }
