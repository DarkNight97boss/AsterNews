...
// ---------------- Uso del menu (per l'interfaccia che si adatta) ----------------
export async function trackNav(userId: string, path: string): Promise<void> { const day = new Date().toISOString().slice(0, 10); await run('INSERT INTO nav_usage (user_id, path, day, count) VALUES (?,?,?,1) ON CONFLICT (user_id, path, day) DO UPDATE SET count = nav_usage.count + 1', [userId, path, day]); if (Math.random() < 0.02) await run('DELETE FROM nav_usage WHERE day < ?', [new Date(Date.now() - 120 * 86400000).toISOString().slice(0, 10)]); }
export async function navUsage(userId: string, days = 30): Promise<{ used: string[]; firstDay: string | null }> { const from = new Date(Date.now() - days * 86400000).toISOString().slice(0, 10); const rows = (await all('SELECT DISTINCT path FROM nav_usage WHERE user_id = ? AND day >= ?', [userId, from])) as Row[]; const first = (await get('SELECT MIN(day) d FROM nav_usage WHERE user_id = ?', [userId])) as Row | undefined; return { used: rows.map((r) => String(r.path)), firstDay: first?.d ? String(first.d) : null }; }

// ---------------- Chiavi d'invito alle cerchie ----------------
const rowToInvite = (r: Row): import('./models').CircleInvite => ({ token: String(r.token), circleId: String(r.circle_id), maxUses: Number(r.max_uses ?? 1), uses: Number(r.uses ?? 0), revoked: !!r.revoked, createdAt: String(r.created_at ?? '') });
export async function insertCircleInvite(i: import('./models').CircleInvite): Promise<void> { await run('INSERT INTO circle_invites (token, circle_id, max_uses, uses, revoked, created_at) VALUES (?,?,?,?,0,?)', [i.token, i.circleId, i.maxUses, i.uses, i.createdAt]); }
export const findCircleInvite = async (token: string): Promise<import('./models').CircleInvite | undefined> => { const r = (await get('SELECT * FROM circle_invites WHERE token = ?', [token])) as Row | undefined; return r ? rowToInvite(r) : undefined; };
export async function useCircleInvite(token: string): Promise<void> { await run('UPDATE circle_invites SET uses = uses + 1 WHERE token = ?', [token]); }
export async function revokeCircleInvite(token: string): Promise<void> { await run('UPDATE circle_invites SET revoked = 1 WHERE token = ?', [token]); }
export const listCircleInvites = async (): Promise<import('./models').CircleInvite[]> => (await all('SELECT * FROM circle_invites ORDER BY created_at DESC LIMIT 100') as Row[]).map(rowToInvite);
export const readersInCircles = async (): Promise<{ id: string; name: string; email: string; circles: string[] }[]> => (await all("SELECT id, name, email, prefs FROM readers WHERE prefs LIKE '%\"circles\":[\"%' LIMIT 500") as Row[]).map((r) => { let c: string[] = []; try { c = (JSON.parse(String(r.prefs ?? '{}')).circles ?? []) as string[]; } catch { c = []; } return { id: String(r.id), name: String(r.name ?? ''), email: String(r.email), circles: c }; });
