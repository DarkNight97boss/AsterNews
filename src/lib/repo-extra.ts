...
const rowToNote = (r: Row): ArticleNote => ({ id: String(r.id), articleId: String(r.article_id), userId: String(r.user_id ?? ''), kind: (r.kind as ArticleNote['kind']) ?? 'note', body: String(r.body ?? ''), resolved: !!r.resolved, createdAt: String(r.created_at ?? '') });
export async function insertNote(n: ArticleNote): Promise<void> { await run('INSERT INTO article_notes (id, article_id, user_id, kind, body, resolved, created_at) VALUES (?,?,?,?,?,?,?)', [n.id, n.articleId, n.userId, n.kind, n.body, b(n.resolved), n.createdAt]); }
...
// ---------------- Newsletter, backup ----------------
export async function insertNewsletterSend(s: NewsletterSend): Promise<void> { await run('INSERT INTO newsletter_sends (id, subject, kind, recipients, sent_at, status, message) VALUES (?,?,?,?,?,?,?)', [s.id, s.subject, s.kind, s.recipients, s.sentAt, s.status, s.message]); }
export const listNewsletterSends = async (limit = 30): Promise<NewsletterSend[]> => (await all('SELECT * FROM newsletter_sends ORDER BY sent_at DESC LIMIT ?', [limit]) as Row[]).map((r) => ({ id: String(r.id), subject: String(r.subject ?? ''), kind: String(r.kind ?? ''), recipients: Number(r.recipients ?? 0), sentAt: String(r.sent_at ?? ''), status: String(r.status ?? ''), message: String(r.message ?? '') }));
