...
export async function toggleSeedPublicAction(id: string): Promise<ActionResult> { const u = await requireUser(); const r = await findRecord<{ text: string; public?: boolean }>(id); if (!r || r.owner !== u.id) return { ok: false, message: 'Appunto non trovato.' }; const { updateRecord } = await import('./records'); await updateRecord(id, { data: { ...r.data, public: !r.data.public } }); revalidatePath('/admin/officina'); revalidatePath('/quaderno'); return { ok: true, message: r.data.public ? 'Appunto tornato privato.' : 'Appunto visibile nel Quaderno pubblico.' }; }
