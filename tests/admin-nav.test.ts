...
describe('menu che si adatta', () => {
  it('il quotidiano in squadra vede quasi tutto in primo piano', () => { const r = arrangeNav(NAV, { profile: 'quotidiano', autoHide: true }, { solo: false, used: [], mature: false }); expect(hrefs(r.more)).toEqual(['/admin/personale']); });
  ...
  it('con autoHide spento non sparisce nulla del profilo', () => { expect(hrefs(arrangeNav(NAV, { profile: 'quotidiano', autoHide: false }, { solo: false, used: [], mature: true }).more)).toEqual(['/admin/personale']); });
