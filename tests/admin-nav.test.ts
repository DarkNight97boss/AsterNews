...
describe('menu che si adatta', () => {
  it('il quotidiano in squadra vede quasi tutto in primo piano', () => { const r = arrangeNav(NAV, { profile: 'quotidiano', autoHide: true }, { solo: false, used: [], mature: false }); expect(r.more).toHaveLength(0); });
  it('il blog personale da soli: niente desk, zone, annunci, utenti in primo piano', () => {
    ...
    expect(r.main.length).toBeLessThan(26);
  });
  ...
  it('con autoHide spento non sparisce nulla del profilo', () => { expect(hrefs(arrangeNav(NAV, { profile: 'quotidiano', autoHide: false }, { solo: false, used: [], mature: true }).more)).toHaveLength(0); });
