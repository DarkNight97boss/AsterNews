...
test('feed, API e sitemap rispondono', async ({ request }) => { for (const u of ['/feed.xml', '/api/v1/site', '/sitemap.xml', '/feed/google-news.xml', '/archivio', '/pubblicita', '/notifiche', '/podcast', '/storie', '/trasparenza', '/correzioni', '/previsioni', '/domande-aperte', '/attenzione', '/riprendi', '/colazione', '/mappa', '/percorsi']) { const r = await request.get(u); expect(r.status(), u).toBe(200); } });
