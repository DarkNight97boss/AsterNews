...
// In CI si usano gli account demo (DEMO_MODE=1, password unica). In locale: E2E_EMAIL ed E2E_PASSWORD, altrimenti il test di login viene saltato.
const EMAIL = process.env.E2E_EMAIL ?? (process.env.CI ? 'direttore@asternews.it' : '');
const PASSWORD = process.env.E2E_PASSWORD ?? (process.env.CI ? 'aster2026' : '');
