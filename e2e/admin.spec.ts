...
test.describe('redazione', () => {
  ...
  test('comunità: una domanda al Comune passa dalla moderazione, parte il contatore, arriva la risposta; la bacheca si risolve col link', async ({ page }) => {
    const q = `Quando riapre la piscina comunale e2e ${Date.now()}?`; await page.goto('/domande-al-comune'); await page.getByText('Fai una domanda al Comune').click(); await page.getByLabel(/La domanda, in una frase chiara/).fill(q); await page.getByRole('button', { name: 'Metti in coda' }).click(); await expect(page.getByText(/contiamo l.attesa in pubblico/)).toBeVisible({ timeout: 15_000 });
    await page.reload(); await expect(page.getByText(q)).toBeHidden();
    await page.goto('/admin/partecipazione'); const item = page.locator('.reply-item', { hasText: q }); await item.getByRole('button', { name: 'Pubblica' }).click(); await expect(item).toBeHidden({ timeout: 15_000 });
    await page.goto('/admin/comunita'); const row = page.locator('.reply-item', { hasText: q }); await row.getByLabel('Inoltrata il').fill(new Date(Date.now() - 9 * 86_400_000).toISOString().slice(0, 10)); await row.getByRole('button', { name: /parte il contatore/ }).click(); await expect(row.getByLabel('Risposta del Comune')).toBeVisible({ timeout: 15_000 });
    await page.goto('/domande-al-comune'); await expect(page.locator('.council-queue li', { hasText: q }).locator('.wait b')).toHaveText('9');
    await page.goto('/admin/comunita'); const row2 = page.locator('.reply-item', { hasText: q }); await row2.getByLabel('Risposta del Comune').fill('Riapre il primo ottobre, dopo il collaudo.'); await row2.getByRole('button', { name: 'Pubblica la risposta' }).click(); await expect(row2).toBeHidden({ timeout: 15_000 });
    await page.goto('/domande-al-comune'); await expect(page.getByText('Riapre il primo ottobre, dopo il collaudo.')).toBeVisible();
    const title = `Mazzo di chiavi e2e ${Date.now()}`; await page.goto('/bacheca'); await page.getByText('Metti un avviso in bacheca').click(); await page.getByLabel(/^Tipo/).selectOption('Ho perso'); await page.getByLabel(/In poche parole/).fill(title); await page.getByLabel(/Dettagli/).fill('Perso ieri sera in piazza, portachiavi rosso.'); await page.getByLabel(/Come contattarti/).fill('333 0000000'); await page.getByRole('button', { name: /Pubblica l.avviso/ }).click();
    const link = (await page.locator('.contrib-done code').innerText()).trim(); await page.goto('/admin/partecipazione'); await page.locator('.reply-item', { hasText: title }).getByRole('button', { name: 'Pubblica' }).click(); await page.goto('/bacheca'); await expect(page.locator('.board:not(.solved)').getByText(title)).toBeVisible();
    await page.goto(new URL(link).pathname + new URL(link).search); await expect(page.getByText(/Segnato come risolto/)).toBeVisible({ timeout: 15_000 }); await page.goto('/bacheca'); await expect(page.locator('.board.solved').getByText(title)).toBeVisible();
  });
  test('distribuzione e sostenibilità: forma breve, llms.txt, redirect per l\'uscita, metriche velate, CO₂ della pagina', async ({ page }) => {
    const llms = await (await page.request.get('/llms.txt')).text(); expect(llms).toContain('## Come citarci'); expect(llms).toContain('/api/verify/');
    const red = await page.request.get('/api/export/redirects?formato=netlify'); expect(red.status()).toBe(200); expect(await red.text()).toMatch(/^\/\S+  \/\S+\/  301$/m);
    await page.goto('/admin/articoli'); const href = await page.locator('a[href^="/admin/articoli/a"]').first().getAttribute('href'); const id = href!.split('/').pop()!; await page.goto(`/breve/${id}`); const text = await page.locator('.short-text').innerText(); expect(text.length).toBeLessThanOrEqual(306); await expect(page.getByRole('link', { name: /Manda su WhatsApp/ })).toHaveAttribute('href', /^https:\/\/wa\.me\/\?text=/);
    await page.goto('/admin/ritmi'); await page.getByRole('button', { name: /Vela i numeri per 7 giorni/ }).click(); await expect(page.getByText(/Attivo fino al/)).toBeVisible(); await page.goto('/admin'); await expect(page.locator('body')).toHaveClass(/no-metrics/); await page.goto('/admin/ritmi'); await page.getByRole('button', { name: /Torna a vedere i numeri/ }).click(); await expect(page.locator('body')).not.toHaveClass(/no-metrics/);
    await page.goto('/'); await expect(page.locator('.eco-badge')).toContainText(/g di CO₂/, { timeout: 15_000 });
  });
});
